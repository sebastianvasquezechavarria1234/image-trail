...
export default function ImageTrail({ items = [], variant = 1 }) {
  ...
  useEffect(() => {
    ...
    // Cleanup is handled by page reload or component unmount,
    // though typically the animation loops run via requestAnimationFrame.
  }, [variant, items]);
  ...
  return (
    <div className="w-full h-full relative z-[100] rounded-lg bg-transparent overflow-visible" ref={containerRef}>
      {items.map((url, i) => (
        <div
          className="content__img w-[190px] h-[300px] rounded-none absolute top-0 left-0 opacity-0 overflow-hidden [will-change:transform,filter]"
          key={i}
        >
          <div
